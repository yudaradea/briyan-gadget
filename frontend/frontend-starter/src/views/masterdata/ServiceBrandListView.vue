...
const columns = [{ key: "nama", label: "Nama Merk HP Service" }];

async function fetchBrands(params) {
    const { data } = await api.get("/service-brands", { params });
    return data.data;
}
...
async function saveForm() {
    ...
    try {
        if (editId.value) {
            await api.put(`/service-brands/${editId.value}`, form.value);
        } else {
            await api.post("/service-brands", form.value);
        }
...
async function doDelete() {
    deleting.value = true;
    try {
        await api.delete(`/service-brands/${deleteId.value}`);
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Kelola data merk HP untuk layanan service
                </p>
            </div>
            ...
            >
                ...
                >
                    <h3 class="mb-4 text-lg font-semibold text-gray-900">
                        {{ editId ? "Edit" : "Tambah" }} Merk Service
                    </h3>
