...
const error = ref("");
const form = ref({ nama: "", keterangan: "" });

const columns = [
    { key: "nama", label: "Nama Grade" },
...
async function saveForm() {
    ...
    error.value = "";
    try {
        if (editId.value) await api.put(`/grades/${editId.value}`, form.value);
        else await api.post("/grades", form.value);
...
async function doDelete() {
    deleting.value = true;
    try {
        await api.delete(`/grades/${deleteId.value}`);
...
<template>
    <div>
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Grade</h1>
                <p class="mt-1 text-sm text-gray-500">
                    Kelola klasifikasi kondisi barang
                </p>
            </div>
            <button
                @click="openCreate"
