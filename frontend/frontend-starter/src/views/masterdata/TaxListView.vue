...
async function saveForm() {
    ...
    error.value = "";
    try {
        if (editId.value) await api.put(`/taxes/${editId.value}`, form.value);
        else await api.post("/taxes", form.value);
...
async function doDelete() {
    deleting.value = true;
    try {
        await api.delete(`/taxes/${deleteId.value}`);
